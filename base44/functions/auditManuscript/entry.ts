...
const FILE_URL = "https://media.base44.com/files/public/69cc495ef72e0f794cb78a95/b325182ef_SGTGEORGERAMOSTheMathematicsofVietnam11.docx";
...
const CHAPTER_LIST = [
  ...
  { num: 46, title: "Epilogue: The Name on the Record",  act: "III" },
];
...
Deno.serve(async (req) => {
  ...
  // Handle missing chapter 39
  if (chapterNum === 39 && false) { // re-check in v11 — may no longer be missing
    await base44.entities.ManuscriptAuditResult.update(record.id, {
      ...
      continuity_issues: ["Chapter 39 is entirely absent from the manuscript. This is the only missing chapter."],
      summary: "Chapter 39 is completely missing from the manuscript. This is a CRITICAL gap in Act III that breaks narrative continuity.",
      priority_fix: "Write Chapter 39 — it is the only absent chapter in the entire 44-chapter manuscript.",
      status: "complete"
    });
    return Response.json({ success: true, chapter: 39, status: "complete" });
  }
  ...
  if (runs.length > 0) {
    ...
    const isComplete = complete.length >= 46;
