...
import CharacterArcConsistency from "./CharacterArcConsistency";
  ...
  return (
    <div className="animate-slide-up space-y-4">
      ...
      <div className="flex gap-1">
        {[{id:"profiles",label:"CHARACTER PROFILES"},{id:"arcconsistency",label:"📈 ARC CONSISTENCY"}].map(t => (
          <button key={t.id} onClick={() => setSubTab(t.id)}
            className={`text-[10px] font-bold font-mono px-3 py-1.5 rounded border transition-all ${subTab === t.id ? "bg-accent text-white border-accent" : "border-border text-muted-foreground hover:text-foreground"}`}>
            {t.label}
          </button>
        ))}
      </div>

      {subTab === "arcconsistency" && <CharacterArcConsistency />}
