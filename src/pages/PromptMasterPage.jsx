  ...
  return (
    <div className="animate-slide-up">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 mb-4">
        {[
          { label: "CURRENT Ω [UNVERIFIED]", val: ch.oc.toFixed(2), color: "text-foreground" },
          { label: "PROJECTED Ω [UNVERIFIED]", val: ch.op.toFixed(2), color: ch.ok ? "text-teal" : "text-gold" },
          { label: "Ω GAP (construct only)", val: ch.gap <= 0 ? "ON TARGET" : `+${ch.gap.toFixed(2)}`, color: ch.gap <= 0 ? "text-teal" : ch.gap < 0.5 ? "text-gold" : "text-red" },
      ...
      <div className="mb-4">
        ...
        <div className="text-[8px] text-muted-foreground mt-1">
          {ch.ok
            ? "Projected on target (Ω = [UNVERIFIED CONSTRUCT] — not a quality verdict)"
            : `Gap of ${ch.gap.toFixed(2)} remains (construct only — verify with AutoCrit)`}
        </div>
      </div>
              ...
              return (
                ...
                >
                  ...
                  <div className={`text-[8px] font-mono font-bold ${ch.ok ? "text-teal" : "text-gold"}`}>
                    Ω {ch.oc.toFixed(2)}→{ch.op.toFixed(2)} <span className="text-[6px] text-muted-foreground">[UNVERIFIED]</span>
                  </div>
                  {ch.bug > 0 && (
...
function StatsScreen() {
  ...
  return (
    <div className="animate-slide-up space-y-5">
      ...
      <div className="mt-3 p-3 bg-orange/5 border border-orange/30 rounded-lg space-y-1.5 text-[8px] font-mono">
        <div className="text-orange font-bold">SPSS GOVERNANCE — A7/A8/A9 (SGTRamos_SPSS_Tracking_HONEST_v1)</div>
        <div className="text-foreground/70">A7: Omega and component scores are [UNVERIFIED CONSTRUCTS] pending external validation. Verified: word count, dialogue %, readability, passive %, code-switch %.</div>
        {(() => {
          const eliteCount = CHAPTER_DATA.filter(c => c.tier === "OMEGA ELITE").length;
          const pct = Math.round((eliteCount / CHAPTER_DATA.length) * 100);
          return eliteCount === CHAPTER_DATA.length
            ? <div className="text-red font-bold">⛔ A8 ANTI-SCORE-TO-TARGET: 100% of chapters at same tier — likely scoring-to-target, not measurement. Re-run with external instrument before reporting tiers.</div>
            : pct > 70
            ? <div className="text-orange font-bold">⚠ A8 WARNING: {pct}% uniform-tier distribution — scoring-to-target risk. Validate with AutoCrit before citing tiers.</div>
            : null;
        })()}
        <div className="text-foreground/70">A9: Pairs 17/18 (~90%), 14/22 (~86%), 26/27 (~67%) are DUP-contaminated. Chapter-level Omega scores suppressed for these pairs until author resolves canonical version.</div>
      </div>
    </div>
  );
}
...
function HelpScreen() {
  return (
    <div className="text-[10px] font-mono text-muted-foreground leading-loose p-2">
      ...
      <div className="mb-3 text-red/70">Pulitzer probability: <span className="font-bold">RETIRED</span> — no defensible basis (see workbook Sheet 2)</div>
    ...
    <div className="min-h-screen bg-background font-mono">
      ...
      <div className="border-b border-border bg-card/90 backdrop-blur-sm sticky top-0 z-30">
        <div className="max-w-5xl mx-auto px-4 md:px-6 h-14 flex items-center gap-4">
          ...
          <div className="text-[8px] font-mono text-orange/80 bg-orange/5 border border-orange/20 rounded px-2 py-0.5 mt-0.5">
            ⚠ SPSS GOVERNANCE: Omega &amp; component scores = [UNVERIFIED CONSTRUCT]. Verified fields: word count, dialogue %, readability, passive %, code-switch %.
          </div>
          </div>
